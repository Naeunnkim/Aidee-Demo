...
// AI 답변에서 페르소나 정보를 추출하는 정규식 기반 헬퍼 함수
const parsePersonaData = (content: string) => {
  // 1. 이미지 URL 추출
  const imageUrlMatch = content.match(/https?:\/\/[^\s]+(?:\.jpg|\.png|\.webp|\/seed\/[^\s]+)/);
  const imageUrl = imageUrlMatch ? imageUrlMatch[0] : "";

  // 2. 각 섹션별 데이터 추출 (프롬프트의 글머리 기호 규칙 활용)
  ...
  const successData = successLines.map(line => {
    ...
    return { tag: tag.replace('#', ''), desc: descParts.join(' ') };
  });
    ...
    <div className="flex h-screen bg-white overflow-hidden w-full">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white items-center">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto w-full max-w-4xl px-6 py-8 space-y-6 scrollbar-hide">
          ...
          {messages.map((m) => {
            // 💡 페르소나 카드 판별 로직: AI 답변이고 'Persona Card'라는 핵심 키워드가 포함된 경우
            const isPersonaCard = m.role === 'assistant' && m.content.includes('Persona Card');

            if (isPersonaCard) {
              const personaData = parsePersonaData(m.content);
              
              // 파싱된 데이터가 최소한의 요건(이미지 URL 등)을 갖췄을 때만 카드 노출
              if (personaData.imageUrl || personaData.user.length > 0) {
                return <PersonaCard key={m.id} data={personaData} />;
              }
            }

            // 일반 메시지 렌더링 (기존 디자인 유지)
            return (
              <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`max-w-[514px] p-5 rounded-[24px] text-base font-medium leading-relaxed shadow-sm ${
                  m.role === 'user' 
                  ? 'bg-gray-100 text-neutral-900 rounded-tr-none' 
                  : 'bg-gray-200 text-neutral-900 rounded-tl-none'
                }`}>
                  {/* prose-p:my-5: 문단(p) 위아래 간격을 더 넓게(20px 정도) 설정
                      prose-p:leading-7: 줄 간격을 넓혀서 가독성 향상
                  */}
                  <div className="prose prose-sm max-w-none break-words prose-p:my-5 prose-p:leading-7 prose-li:my-2 prose-headings:mb-4">
                    <ReactMarkdown 
